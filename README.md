...
* Tried and reverted: giving each statistics counter its own cache line measured **6.5% slower** on an 8-thread announce benchmark, not faster. An announce writes about five counters, and padding turns one cache-line fetch into five; the invalidation traffic it avoids does not pay for that at this core count.
