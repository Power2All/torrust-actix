...
pub fn api_service_routes(data: Arc<TorrentTracker>, object: ApiTrackersConfig) -> Box<dyn Fn(&mut ServiceConfig)>
{
...
pub async fn api_service_stats_get(request: HttpRequest, remote_ip: RemoteIP, data: Data<Arc<TorrentTracker>>, object: Data<&ApiTrackersConfig>) -> HttpResponse
{
    api_service_stats_log(remote_ip.0, data.clone(), object).await;

    let params = web::Query::<QueryToken>::from_query(request.query_string()).unwrap();
    if let Some(response) = api_service_token(params.token.clone(), data.config.clone()).await {
        return response;
    }
...
) -> (ServerHandle, impl Future<Output=Result<(), std::io::Error>>)
{
    ...
    if api_server_object.ssl.unwrap() {
        ...
        let certs_file = &mut BufReader::new(File::open(api_server_object.clone().ssl_key.unwrap()).unwrap());
        let key_file = &mut BufReader::new(File::open(api_server_object.clone().ssl_cert.unwrap()).unwrap());
        ...
        let server = HttpServer::new(move || {
            App::new()
                ...
                .configure(api_service_routes(data.clone(), api_server_object.clone()))
        })
...
pub async fn api_service_stats_log(ip: IpAddr, tracker: Data<Arc<TorrentTracker>>, _object: Data<&ApiTrackersConfig>)
{
    if ip.is_ipv4() {
...
pub async fn api_service_retrieve_remote_ip(request: HttpRequest, _data: Data<Arc<TorrentTracker>>, object: Data<&ApiTrackersConfig>) -> Result<IpAddr, ()>
{
...
pub async fn api_validate_ip(request: HttpRequest, data: web::Data<Arc<TorrentTracker>>, object: Data<&ApiTrackersConfig>) -> Result<IpAddr, HttpResponse>
{
    match api_service_retrieve_remote_ip(request.clone(), data.clone(), object.clone()).await {
        Ok(ip) => {
            api_service_stats_log(ip, data.clone(), object).await;
            Ok(ip)
        }
        Err(_) => {
...
pub async fn api_service_not_found(request: HttpRequest, data: web::Data<Arc<TorrentTracker>>, object: Data<&ApiTrackersConfig>) -> HttpResponse
{
    let _ = match api_validate_ip(request.clone(), data.clone(), object).await {
        Ok(ip) => ip,
        Err(result) => {
            return result;
        }
