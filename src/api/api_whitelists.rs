...
pub async fn api_service_whitelists_get(request: HttpRequest, data: Data<Arc<ApiServiceData>>) -> HttpResponse
{
    // Validate client
    ...
    HttpResponse::Ok().content_type(ContentType::json()).json(json!({
        "status": "ok",
        "whitelists": data.torrent_tracker.get_whitelist()
    }))
}

pub async fn api_service_whitelists_post(request: HttpRequest, path: web::Path<(String,)>, mut payload: web::Payload, data: Data<Arc<ApiServiceData>>) -> HttpResponse
{
    // Validate client
    if let Some(error_return) = api_validation(&request, &data).await { return error_return; }
    ...
    for info_hash_torrent in hashes {
        if info_hash_torrent.len() == 40 {
            ...
            match data.torrent_tracker.add_whitelist(info_hash) {
                ...
                false => { torrents_output.insert(info_hash, json!({"status": "already whitelisted"})); }
            }
        }
    }
