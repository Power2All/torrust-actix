...
use sqlx::{ConnectOptions, Error, Pool, Row, Sqlite};
...
impl DatabaseConnectorSQLite {
    ...
    pub async fn load_torrents(&self, tracker: Arc<TorrentTracker>) -> Result<(u64, u64), Error>
    {
        ...
        while let Some(result) = rows.try_next().await? {
            ...
            torrents_parsing.insert(InfoHash(info_hash), TorrentEntryItem { completed: completed_data, seeders: 0, leechers: 0, });
            counter += 1;
            total_torrents += 1;
