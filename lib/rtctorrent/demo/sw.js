 ...
 *    won't send further Range requests.
...
// streamId → ReadableStreamDefaultController
const streamControllers = new Map();

// Chunks that arrive via postMessage before the fetch handler fires
...
self.addEventListener('fetch', event => {
    ...
    // Duplicate request — a stream is already live for this streamId.
    // Return 416 so the browser knows to keep reading the first stream.
    ...
    // First request (Range OR non-Range): respond with 200 + full stream.
    // We intentionally ignore any Range header here — responding with 200
    // (instead of 206 or 416) signals to Chrome that range requests are not
    // supported, and Accept-Ranges: none reinforces that.
    ...
    const body = new ReadableStream({
        start(controller) {
            ...
            // Flush any chunks that arrived before this handler ran
...
self.addEventListener('message', event => {
    ...
    if (!controller) {
        // Stream not yet set up — buffer until the fetch handler is ready
