...
if (filePaths.length === 1 && !torrentName) {
    // Single-file torrent: info.name IS the filename — keep the extension so
    // leechers can identify the media type (e.g. isPlayable('.mp4') → true).
    torrentName = path.basename(filePaths[0]);
    outFile = outFile || path.basename(filePaths[0], path.extname(filePaths[0])) + '.torrent';
} else {
    // Multi-file torrent: name is a directory — no extension needed.
