...
class RtcTorrent {
    ...
    async create(files, options = {}) {
        ...
        if (options.webseedUrls) {
            const urls = Array.isArray(options.webseedUrls) ? options.webseedUrls : [options.webseedUrls];
            exportable['url-list'] = urls.length === 1 ? urls[0] : urls;
        }
    ...
    async parseTorrentFile(torrentData) {
        ...
        const webseeds = urlList
            ...
            : [];
        return { info, announce: decoded.announce, announceList: decoded['announce-list'], infoHash, name, webseeds };
    }
...
class Torrent {
    constructor(client, torrentData, options) {
        ...
        this._peerStats = new Map(); // peer_id → { pendingRequests, responseTimes, blacklisted }
    ...
    parseBencodedResponse(responseBytes) {
        try {
            ...
            if (decoded['failure reason']) {
                ...
                // Tracker responded successfully but without any RtcTorrent fields —
                // it's a standard BitTorrent tracker that doesn't support WebRTC signaling
                result.rtcNotSupported = true;
            }
            return result;
        } catch (err) {
        ...
        try {
            ...
            if (channel.bufferedAmount > 512 * 1024) {
                await new Promise(resolve => {
                    ...
                    // Primary: event-based (fast in browsers)
                    channel.addEventListener('bufferedamountlow', resolve, { once: true });
                    // Fallback: polling every 10 ms — needed when bufferedamountlow
                    // is not dispatched (e.g. @roamhq/wrtc Node.js native module)
                    const poll = () => {
    ...
    _blacklistPeer(peerId, reason) {
        ...
        // Re-queue any pieces still waiting on this peer so they get
        // reassigned to a faster peer on the next _requestMissingPieces() call.
    ...
    _readRangeFromDisk(filePaths, start, end) {
        ...
        for (const fileMeta of filePaths) {
            ...
            // Reuse cached fd — opening a file on every piece is very slow on WSL/Windows
    ...
    handlePieceData(pieceIndex, pieceData, peerInfo) {
        ...
        this._verifyPiece(pieceIndex, pieceData).then(ok => {
            ...
            // Record response time for peer speed tracking
    ...
    async requestPieceFromPeers(pieceIndex) {
        ...
        new DataView(frame.buffer).setUint32(1, pieceIndex, false);
        // Collect open channels from non-blacklisted peers, round-robin by piece index.
        const openPeers = [];
        ...
        if (openPeers.length > 0) {
            ...
            try {
                ...
                sent = true;
                // Record the send time for speed tracking
    ...
    async streamVideo(fileIndex = 0, videoElement, opts = {}) {
        ...
        // Prefer Service Worker streaming: truly seamless, no reload, any format
        if (navigator.serviceWorker?.controller) {
            try {
                console.log('[RtcTorrent] Service Worker active — using seamless SW stream');
        ...
        // Fragmented MP4 / WebM: use MSE (append without reload)
        ...
        // Regular faststart MP4: progressive blob (requires periodic reload to extend)
    ...
    async _streamWithSW(file, videoElement, opts = {}) {
        ...
        // The SW stream is linear (no Range support), so seeking is broken while
        // it's active.  Now that everything is buffered in this.pieces, switch
        // the video element to a Blob URL — browsers support full random-access
        // seeking on blob: URLs.
        try {
            ...
            if (videoElement.src.includes(streamId)) {
                ...
                console.log('[RtcTorrent] Switched to Blob URL — seeking now available');
            }
        } catch (e) {
            console.warn('[RtcTorrent] Could not switch to Blob URL for seeking:', e.message);
        }
    }
    ...
    async _streamFaststart(file, videoElement, opts = {}) {
        ...
        const INITIAL_EXTRA = initialExtra;  // Pieces after moov before playback starts (~3.2 MB at default 50 × 64 KB)
        const BUFFER_AHEAD_S = 5;   // Rebuild blob when less than this many seconds remain in current blob
        const REBUILD_MIN_GAIN = 20; // Don't rebuild unless at least this many new pieces are available
        ...
        for (let i = startPiece; i <= initialEndPiece; i++) {
            await this._waitForPiece(i);
            if (typeof onProgress === 'function') {
                // Show 0→100% relative to the initial buffer, not the whole file
        ...
        const checkIntervalId = setInterval(async () => {
            ...
            // Gap recovery: if the contiguous run hasn't advanced past the current blob end,
            // the next required piece may be stuck in a stale request — force re-request it.
            if (latest <= lastBlobPiece) {
            ...
            if (bufferedAhead < BUFFER_AHEAD_S && latest > lastBlobPiece + REBUILD_MIN_GAIN) {
    ...
    async _waitForPiece(index) {
        ...
        while (!this.pieces.has(index)) {
            if (!this.requestedPieces.has(index) || sinceLastRequest >= RETRY_MS) {
                // Clear any stale entry so the piece is re-sent to all open channels
                this.requestedPieces.delete(index);
                this.requestPieceFromPeers(index);
                this.requestedPieces.add(index);
                sinceLastRequest = 0;
