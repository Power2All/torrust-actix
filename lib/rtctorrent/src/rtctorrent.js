...
const MSG_HAVE = 0x03;
// Chunked delivery from Rust seeder (piece too large for one SCTP message).
// Frame: [0x04][4-byte piece_index][4-byte total_size][4-byte offset][chunk_data]
...
class Torrent {
    constructor(client, torrentData, options) {
        ...
        this._pieceChunks = new Map(); // piece_index → { total, buf, received } — for chunked delivery
    ...
    handleMessage(data, peerInfo, channel) {
        try {
            ...
            if (msgType === MSG_PIECE_REQUEST) {
                ...
                // Chunked delivery: [0x04][4-byte piece_index][4-byte total_size][4-byte offset][chunk_data]
    ...
    async _servePiece(pieceIndex, channel) {
        ...
        try {
            ...
            // Decide whether to send as a single MSG_PIECE_DATA or as chunked MSG_PIECE_CHUNK
            // frames.  Browsers handle SCTP fragmentation transparently so we could always
            // send one large message, but chunking keeps us compatible with webrtc-rs (Rust)
            // leechers and avoids SCTP stack quirks in @roamhq/wrtc on Windows/WSL2.
            const SCTP_MAX_PAYLOAD = 65531; // 65536 SCTP limit − 5 byte frame header
    ...
    async streamVideo(fileIndex = 0, videoElement, opts = {}) {
        ...
        // 1. Service Worker: seamless streaming for ALL video types.
        //    Bytes flow through a ReadableStream — src is set once, never replaced.
        //    After the stream closes (all pieces received), the browser has the full
        //    file buffered and seeks within that range without any new network requests.
    ...
    /**
     * Stream a faststart MP4 by appending pieces directly to a MediaSource SourceBuffer.
     * Unlike blob-URL replacement, appendBuffer() never interrupts playback.
     * The browser parses the moov atom from the first append and can play immediately;
     * as more pieces arrive they are fed into the live decode session.
     * endOfStream() at the end locks in the correct duration for full post-download seeking.
     */
