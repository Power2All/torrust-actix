...
async fn main() {
    ...
    if let Some(yaml_path) = cli.torrents.clone() {
        ...
        run_torrents_mode(yaml_path, cli_proxy, cli_web).await;
    } else {
        ...
        if let Some(web_port) = cli.web_port {
            ...
            spawn_web_server(web_cfg, yaml_path, sf, ss, reload_tx);
        }
...
async fn run_torrents_mode(yaml_path: PathBuf, cli_proxy: Option<ProxyConfig>, cli_web: WebConfig) {
    println!("=== RtcTorrent Seeder (Rust native, multi-torrent mode) ===");
    println!("Config  : {}", yaml_path.display());
    println!();
    if !yaml_path.exists() {
        println!("[rtc-seed] Creating empty config file: {}", yaml_path.display());
    ...
    loop {
        let (file, entries) = match load_yaml_entries(&yaml_path, cli_proxy.as_ref()) {
            Ok(e) => e,
            Err(e) => {
                eprintln!("[rtc-seed] Failed to load {}: {}", yaml_path.display(), e);
                std::process::exit(1);
        ...
        if entries.is_empty() {
            println!("[rtc-seed] No enabled torrent entries — waiting for changes…");
        } else {
        ...
        let should_reload = 'wait: loop {
            ...
            tokio::select! {
                ...
                _ = reload_rx.changed() => {
                    println!("[rtc-seed] Web UI triggered reload…");
                    break 'wait true;
                },
